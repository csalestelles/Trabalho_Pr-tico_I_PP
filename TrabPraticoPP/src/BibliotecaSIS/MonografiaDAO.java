...
public class MonografiaDAO 
{
	...
	public String devolver(int codigoUser)
	{
		...
		try
		{
			...
			if(resultSet.first())
			{
				monografia.setCodigo(resultSet.getInt(2));
				MonografiasEmprestadasDAO docsEmprestados = new MonografiasEmprestadasDAO();
				docsEmprestados.devolucao(codigoUser);
				...
				if(localizar())
				{
					...
					if(resultSet.first())
					{
						int exemplaresNovo = Integer.parseInt(resultSet.getString(9))+1;
						monografia.setExemplares(exemplaresNovo+"");
